...
public class TumblingWindowInfractionCountV2Bolt extends BaseWindowedBolt {


	private static final long serialVersionUID = 8257758116690028179L;
	private static final Logger LOG = LoggerFactory.getLogger(TumblingWindowInfractionCountV2Bolt.class);
	...
	public void execute(TupleWindow inputWindow) {
		...
		for(TruckDriver truckDriver: truckDriverInfractions.keySet()) {
			...
			if(infractionDetail != null) {
				LOG.info("Truck Driver["+ truckDriver + "] has the follwing infractions["+infractionDetail+"]");
				collector.emit(new Values(truckDriver.getDriverId(), truckDriver, infractionDetail));
			}
	...
	private boolean isInfractionEvent(String eventType) {
	 ...
	 public void declareOutputFields(OutputFieldsDeclarer declarer) {
		 ...
		 declarer.declare(new Fields("driverId", "truckDriver", "infractionDetail"));
